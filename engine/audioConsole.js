...
(function () {
    ...
    function initAudioConsole() {
        ...
        script.textContent = `
            ...
            window.__akariVoice = assistant;

            assistant.addEventListener('ready', () => {
                window.dispatchEvent(new Event('audioConsoleReady'));
            });
            ...
            assistant.addEventListener('speechstart', () => {
                window.dispatchEvent(new Event('audioConsoleSpeechStart'));
            });
            assistant.addEventListener('speechend', () => {
                window.dispatchEvent(new Event('audioConsoleSpeechEnd'));
            });
            assistant.addEventListener('result', (e) => {
                window.dispatchEvent(new CustomEvent('audioConsoleResult', { detail: e.detail }));
            });
            assistant.addEventListener('processing', () => {
                window.dispatchEvent(new Event('audioConsoleProcessing'));
            });
            assistant.addEventListener('processingend', () => {
                window.dispatchEvent(new Event('audioConsoleProcessingEnd'));
            });
            assistant.addEventListener('speechdiscarded', (e) => {
                window.dispatchEvent(new CustomEvent('audioConsoleSpeechDiscarded', { detail: e.detail }));
            });
        ...
        `;
        // NOTE: the above is incomplete - will be replaced with full file
