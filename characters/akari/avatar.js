...
function detectSadness(text) {
    ...
    return sadnessKeywords.some(word => text.includes(word)) || negations.some(phrase => text.includes(phrase)) || (frownFaces && frownFaces.length > 0);
}
...
function akariRootFromAvatarScript() {
    try {
        var scripts = document.getElementsByTagName('script');
        for (var i = scripts.length - 1; i >= 0; i--) {
            var src = scripts[i].src || '';
            if (/\/characters\/akari\/avatar\.js/i.test(src)) {
                return src.replace(/characters\/akari\/avatar\.js.*$/i, '');
            }
        }
    } catch (e) {}
    return './';
}
var __akariRoot = akariRootFromAvatarScript();
...
imageElem.src = __akariRoot + "characters/akari/emote2/" + nil;
    };
    ...
    var emoteStorage = localStorage.getItem("emote");
     ...
     function checkEmoteChange() {
        ...
        if (newEmote !== emoteStorage) {
            ...
            emoteStorage = newEmote;
        }
    }
    checkEmoteChange();
    setInterval(checkEmoteChange, 1000);
