say('Akari TTS (speecht5 running locally) v1.1mini is active.');
function loadt5s() {
  ...
  script.textContent = `
      ...
      (async function initializeSynthesizer() {
          synthesizer = await pipeline('text-to-speech', '/eligapris/microsoft-speecht5_tts-ONNX', { quantized: true });
