 ...
 ****************************************************************************************************/
          const live2d_settings = {
            // basic settings
            'modelUrl': 'model',                        // Path to the folder where the model is stored, no slash at the end is required
            'tipsMessage': 'waifu-tips.json',           //The path of the message file prompted by the kanban girl can be left blank and not loaded.
            //Model settings
            'modelName': 'hiyori',                      //The default loaded model name is only valid when there is no local record
            'modelStorage': false,                       //Memory model, the last selected model will be loaded next time you open the page
            'modelRandMode': false,                     // Randomly switch models
            'preLoadMotion': true,                      // Whether to preload action data is only valid for the model3 model. Not preloading can improve the loading speed of the model3 model, but it may cause lag when the action is triggered for the first time.
            'tryWebp': true,                            // If the browser supports WebP format, textures in WebP format will be loaded first. For example, the default texture file is klee.8192/texture_00.png.
            // After enabling, klee.8192/texture_00.png.webp will be loaded first. If the file does not exist, it will fallback automatically.
            // Toolbar settings
            'showToolMenu': true,                       // show toolbar
            'canCloseLive2d': false,                     //Display the close billboard button
            'canSwitchModel': false,                     //Show model switching button
            'canSwitchHitokoto': false,                  //Display the one-word switch button
            'canTakeScreenshot': true,                  //Show the poster girl screenshot button
            'canTurnToHomePage': false,                  //Display return to homepage button
            'canTurnToAboutPage': false,                 //Show jump to about page button
            'showVolumeBtn': false,                     // Display the volume control button, for display only, the relevant logic needs to be implemented by yourself
            // Prompt message settings
            'show Hitokoto': false,                       // Display a word when idle
            'hitokotoAPI': '',                          // Yiyan API, optional 'hitokoto.cn' (default), 'lwl12.com', 'jinrishici.com' (ancient poetry), 'fghrsh.net'
            'showWelcomeMessage': true,                 // Display the welcome message when entering the page
            'showCopyMessage': false,                    // Display the copy content prompt. By default, only the copy within the '#articleContent' element is monitored. If your article content is not under this tag, you can search and modify it below.
            'showF12OpenMsg': false,                     //Display console opening prompt
            // Kanban girl style setting
            'live2dHeight': 500,                        // The height of the poster girl, no unit is required
            'live2dWidth': 1000,                         // Kanban girl width, no unit required
            'waifuMinWidth': 'disable',                 //Hide the billboard when the page width is smaller than the specified value, such as 'disable' (disabled), '1040px' is recommended
            'waifuEdgeSide': 'right:0',                 // The direction of the banner girl's edge, such as 'left:0' (0px to the left), 'right:30' (30px to the right), can be overridden by the following model settings
            //Other miscellaneous settings
            'debug': true,                              //Global DEBUG settings
            'debugMousemove': false,                    //Print the pointer movement coordinates on the console, only available when debug is true
            'logMessageToConsole': true,                //Print the notice message on the console
            'l2dVersion': '2.0.0',                      // current version
            'homePageUrl': 'https://rivens.bronya.moe/',  // Home page address, optional 'auto' (automatic), '{URL website address}'
            'aboutPageUrl': 'https://github.com/Konata09/Live2dOnWeb/', //About page address, '{URL address}'
            'screenshotCaptureName': 'photo.png',   //The file name of the poster girl's screenshot, for example 'live2d.png'
        }
        //Model list
        const live2d_models = [
            {
