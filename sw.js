...
self.addEventListener('fetch', (event) => {
  // Never let an exception escape respondWith — Firefox surfaces that as
  // "A service worker intercepted the request and encountered an error."
  event.respondWith((async () => {
    try {
      ...
      try {
        ...
        // Invalid URL (rare); fall through to network without SW logic
      ...
      // Skip non-GET / opaque navigation edge cases that can reject in Firefox
      ...
      if (url.origin === self.location.origin && url.pathname.startsWith('/Akari/Digita')) {
        try {
          ...
          // Cache the network response (clone inside writeThrough); return the final one
          await writeThrough(event.request, response);
          return final;
        } catch {
          const cached = await caches.match(event.request);
          if (cached) return digitaCors ? withCorsHeaders(cached) : cached;
          return new Response(RECOVERY_HTML, { headers: { 'Content-Type': 'text/html' } });
        }
      }
      ...
      try {
        const response = await fetch(event.request);
        if (response.status === 404 && url.origin === self.location.origin) {
          return new Response(NOT_FOUND_HTML, { status: 404, headers: { 'Content-Type': 'text/html' } });
        }
        const wantCors = akariNetCors && url.origin === self.location.origin;
        const final = wantCors ? withCorsHeaders(response) : response;
        await writeThrough(event.request, response);
        return final;
      } catch {
        const cached = await caches.match(event.request);
        if (cached) {
          const wantCors = akariNetCors && url.origin === self.location.origin;
          return wantCors ? withCorsHeaders(cached) : cached;
        }
        // Cross-origin failures (e.g. vosk model offline) must not show recovery HTML
